...
| Метод | Кол-во задач | Время (Mean) | Выделено памяти (Allocated) |
|---|---:|---:|---:|
| **JobScheduler (Наш)** | **1,000** | **13.00 μs** | **- (0 B)** |
| StandardParallelFor | 1,000 | 15.83 μs | 5,959 B |
| StandardTasksWhenAll | 1,000 | 295.37 μs | 72,205 B |

*Наш `JobScheduler` не только обходит стандартный `Parallel.For` по скорости на средних и больших объемах, но и гарантирует абсолютное отсутствие мусора в куче (Heap).*